...
def get_pie_chart(entered_site):
    pie_df = spacex_df
    if entered_site == 'ALL':
        pie = pie_df.groupby('Launch Site')['class'].sum()
        fig = px.pie(pie, values=pie.values, names=pie.index, title=f'{entered_site} chart of successful and failed launches')
        return fig
    else:
        pie_df = pie_df[pie_df['Launch Site'] == entered_site]
        pie = pie_df['class'].value_counts()
        fig = px.pie(pie, values=pie.values, names=pie.index, title=f'{entered_site} chart of successful and failed launches')
        return fig
# TASK 4:
...
def draw_sactter(mass, entered_site):
    df = spacex_df[(spacex_df['Payload Mass (kg)'] >= mass[0]) & (spacex_df['Payload Mass (kg)'] <= mass[1])]
    if entered_site == 'ALL':
        fig = px.scatter(df, x = 'Payload Mass (kg)', y = 'class', color='Booster Version Category', title=f'{entered_site} chart of Successful and failed launch by Payload')
    else:
